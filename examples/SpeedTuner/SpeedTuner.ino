/* SpeedTuner
    This example shows how to use the DDBot library to figure out the right values for tuning speed
   ...
   in 0.05 increments.
    ...
    At each stage, it will output the adjustment value and the adjusted speed to the Serial monitor,
   so that you can determine when it has sufficient speed to accelerate.
...
void setup() {
    // use the maximum possible speed value
    bot.maxSpeed = 100;
    ...
    for (int counter = 0; counter < 20; counter++) {
        ...
        // again, we need to update the speed values to allow the open-loop control to work
